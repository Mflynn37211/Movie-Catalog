...
get '/actors' do
    ...
    @actors = conn.exec('SELECT actors.name, actors.id FROM actors ORDER BY actors.name ASC')
  end
  erb :'/actors/index'
...
get '/movies' do
  db_connection do |conn|
  @movies = conn.exec('SELECT movies.title,movies.id, movies.year, movies.rating, genres.name AS genre, studios.name
